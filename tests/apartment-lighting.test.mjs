...
const original = read("Apartment.glb", root);
const source = JSON.parse(original.subarray(20, 20 + original.readUInt32LE(12)));
const model = JSON.parse(read("Apartment.gltf"));
const lighting = JSON.parse(read("lighting.json"));
const hash = data => createHash("sha256").update(data).digest("hex");

test("apartment bake preserves materials, triangle count, and source references", () => {
  ...
  assert.equal(lighting.sourceSha256, hash(original));
...
test("apartment baked UVs are finite and contained in the atlas", () => {
...
test("apartment bake outputs match their hashes and texture dimensions", () => {

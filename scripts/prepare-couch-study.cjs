...
(async () => {
  fs.mkdirSync('.tools/couch-study', { recursive: true });
  ...
  try {
    ...
    const geometry = await page.evaluate(async () => {
      ...
      const sofa = await loadCouch(scene);
    ...
    fs.writeFileSync('.tools/couch-study/geometry.json', JSON.stringify(geometry));
    execFileSync(process.env.BLENDER || '.tools/blender-4.5.3-windows-x64/blender.exe', ['-b', '-t', '4', '--python', 'scripts/unwrap-couch.py'], { stdio: 'inherit' });
    await page.goto(url);
    const data = await page.evaluate(async () => {
      ...
      scene.useRightHandedSystem = true;
      const sofa = await loadCouch(scene);
      const atlas = await (await fetch('/comparison/couch/atlas.json')).json();
    ...
    fs.writeFileSync('public/comparison/couch/scene.json', JSON.stringify(data));
